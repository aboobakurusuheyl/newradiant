    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-4">
              ...
              <!-- Add more career entries here if needed -->
            </div>
          </div>
        ...
        <div class="space-y-8">
          ...
          <div v-if="player?.contract_start" class="bg-white rounded-lg shadow-sm p-6">
            <h3 class="text-xl font-bold text-gray-900 mb-4">Contract Details</h3>
            <div class="space-y-3">
              <div class="flex justify-between items-center py-2 border-b border-gray-100">
                <span class="text-gray-600">Start Date</span>
                <span class="font-semibold">{{ formatDate(player.contract_start) }}</span>
              </div>
              <div class="flex justify-between items-center py-2 border-b border-gray-100">
                <span class="text-gray-600">End Date</span>
                <span class="font-semibold">{{ formatDate(player.contract_end) || 'Not specified' }}</span>
              </div>
              <div class="flex justify-between items-center py-2 border-b border-gray-100">
                <span class="text-gray-600">Status</span>
                <span :class="player.is_active ? 'text-green-600' : 'text-red-600'" class="font-semibold">
...
const fetchPlayer = async () => {
  try {
    ...
    router.push('/academy/first-team')
  } finally {
    loading.value = false
  }
}
...
const savePlayer = async (playerData) => {
  try {
    await api.put(`/players/${player.value.id}`, playerData)
    await fetchPlayer()
    closePlayerModal()
  } catch (error) {
